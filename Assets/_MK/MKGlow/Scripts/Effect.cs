...
namespace MK.Glow
{
    ...
    internal sealed class Effect
    {
        ...
        private static readonly int _cArgBufferSize = 48;
        ...
        private ComputeShaderVariants.Variants3x8 _presampleComputeVariants = new ComputeShaderVariants.Variants3x8(1);
        private ComputeShaderVariants.Variants3x8 _downsampleComputeVariants = new ComputeShaderVariants.Variants3x8(9);
        private ComputeShaderVariants.Variants3x8 _upsampleComputeVariants = new ComputeShaderVariants.Variants3x8(17);
        ...
        private ComputeShaderVariants.Feature3x1 _feature3x1Setup = new ComputeShaderVariants.Feature3x1(false, false, false);
        ...
        internal void Enable(RenderPipeline renderPipeline)
        {
            ...
            _renderMaterialGeometry = new Material(_resources.sm40GeometryShader) { hideFlags = HideFlags.HideAndDontSave };
            ...
            if(_useGlare)
            {
                 switch(_settings.quality)
                {
                    ...
                    case Quality.Medium:
                        ...
                        _glareScatteringMult = 2;
                    break;
                    /*Mobile quality level disabled for now... reasons see @ quality defintion
                    case Quality.Mobile:
                        _glareIterations = _glareIterationsBase - 1;
                        _glareScatteringMult = 2;
                    break;
        ...
        private void CheckFeatureSupport()
        {
            ...
            if(_settings.allowGlare && Compatibility.CheckGlareFeatureSupport())
                _useGlare = true;
            else
                _useGlare = false;
        ...
        {      
            ...
            //first include always so it dont conflicts with the constant arg buffer
            //if(_useComputeShaders)
            //    _commandBuffer.SetGlobalMatrix(ShaderProperties.viewMatrix.id, _renderingCamera.worldToCameraMatrix);
            ...
            if(_useLensFlare)
            {
                ...
                SetVector(PipelineProperties.ShaderProperties.lensFlareGhostParams, new Vector4(_settings.lensFlareGhostCount, _settings.lensFlareGhostDispersal, _settings.lensFlareGhostFade, ConvertGammaValue(_settings.lensFlareGhostIntensity)));
                SetVector(PipelineProperties.ShaderProperties.lensFlareHaloParams, new Vector3(_settings.lensFlareHaloSize, _settings.lensFlareHaloFade, ConvertGammaValue(_settings.lensFlareHaloIntensity)));
            ...
            if(_useGlare)
            {
                ...
                Vector4 Scattering = new Vector4(_settings.glareSample0Scattering * _glareScatteringMult, _settings.glareSample1Scattering * _glareScatteringMult, _settings.glareSample2Scattering * _glareScatteringMult, _settings.glareSample3Scattering * _glareScatteringMult);
                glareAngles[0] = AngleToDirection(_settings.glareSample0Angle);
                glareAngles[1] = AngleToDirection(_settings.glareSample1Angle);
                glareAngles[2] = AngleToDirection(_settings.glareSample2Angle);
                glareAngles[3] = AngleToDirection(_settings.glareSample3Angle);
        ...
        private void UpdateRenderIndex(ComputeShaderVariants.Variants3x8 variants, ComputeShaderVariants.Feature3x1 features)
        {
        ...
        /// <param name="features"></param>
        /// <param name="renderDimension"></param>
        private void PrepareDraw(int materialPass, ComputeShaderVariants.Variants3x8 variants, bool enableBloom, bool enableLensflare, bool enableGlare, RenderDimension renderDimension)
        {
            if(_useComputeShaders)
            {
                _feature3x1Setup.x = enableBloom;
                _feature3x1Setup.y = enableLensflare;
                _feature3x1Setup.z = enableGlare;

                UpdateRenderIndex(variants, _feature3x1Setup);
                AttachCArgBufferToComputeKernel();
                UpdateComputeShaderThreadGroups(renderDimension);
            }
            else
        ...
        private void DisableRenderKeywords()
        {
            ...
            SetKeyword(MaterialKeywords.Glare, false);
        }
        ...
        private void PreSample()
        {
            ...
            if(_useGlare)
            {
                ...
                _renderKeywordsBundle.Add(MaterialKeywords.Glare);
